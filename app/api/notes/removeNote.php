 ...
 *
 * Función:
 * Elimina una nota de la base de datos especificada por el usuario
 */
...
$id = trim($input['noteId']);
$userId = $_SESSION['user_id'];

// Ejecuta la operación de eliminación de la nota, si todo va bien
// devuelve un mensaje de success
// si no devuelve un error
try {
    $db = Database::connect();
    $stmt = $db->prepare("DELETE FROM notes WHERE user_id=? AND id=?");
